...
static void
RestartAndSwizzleVungle(VungleInterstitialCustomEvent *model, NSDictionary *customInfo)
{
    [VGVunglePub stop];
...
describe(@"VungleInterstitialCustomEvent", ^{
    ...
    context(@"when requesting a Vungle video ad with custom app id", ^{
        beforeEach(^{
            RestartAndSwizzleVungle(model, [NSDictionary dictionaryWithObject:@"CUSTOM_APP_ID" forKey:@"appId"]);
        });
    ...
    context(@"when the delegate does specify a location", ^{
        beforeEach(^{
            ...
            RestartAndSwizzleVungle(model, nil);
        });
